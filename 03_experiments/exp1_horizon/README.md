...
⚠️ **实验二重做之后，这一组的结论要进一步收回。**

那次用 3 个 seed 直接量到的种子方差是 **±17 个百分点**——比这里最大的差距（h=5 对 h=3 的 12 个点）
还大。这三组都是单 seed，所以 **h=5 那个 64% 同样落在噪声里，不能当结论。**

诚实的表述：这一组实验在当前统计强度下**什么都没测出来**。要让它有意义，
每组需要约 45 个 seed（见 [`../../04_analysis/significance.py`](../../04_analysis/significance.py)）。
