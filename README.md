...
- 实验三的塌缩检测成立，但我给的**机制解释是错的**（不是信息没了，塌掉的是信噪比）。
- **尺度是标记，不是原因。** 相关本身很硬（留一法 r 始终在 [+0.87, +0.95]，
  p < 0.0001），但规划器对尺度**严格不变**——代价是 embedding 空间的 MSE、
  选精英是纯排序，整体缩放不改变任何决策。所以尺度必然在代理别的东西，
  而那个东西是什么，这个项目没回答。最自然的候选（信噪比 `pred_loss/scale²`）
  只有边缘证据：r=−0.59、p=0.027，删掉一个点就不显著。
- 实验四是观测不是干预，n=11。**而且我一开始写下的那个"下一步"是无效的**——
  "评估时缩放 embedding"按构造必然得零效应。有效的替代见
  [实验四的下一步](03_experiments/exp4_representation/README.md)：
  评估期噪声注入，或训练期钉死尺度后重跑。
