...
第二组实验我以为在比架构，事后才发现两个"架构"在这个包里逐字节相同，
14 个百分点的差距只是随机初始化不同。这个错误被保留在仓库里，
因为它意外测出了这套设置的噪声底（约 15 个百分点），
而那把尺子反过来约束了其他每一组实验能宣称什么。
...
### 2. 同预算换架构——这一组没做成，但测出了别的

原计划是固定预算只换架构：把 PLDM（le-wm 对比的 baseline 之一）塞进同一个训练循环和规划器。
跑出来 PLDM 预测更差（0.298 vs 0.266）而规划更好（66% vs 52%），
看着像个漂亮的反例。

**它不是。** 事后查证发现三件事：

1. **两者是同一个架构。** `stable_worldmodel` 里 `pldm/module.py` 和 `lewm/module.py`
   逐字节相同；两个主文件只差 `rollout()` 的推理优化，不涉及结构。
2. **随机初始化不同且无种子控制。** `train.py` 调 `spt.Manager` 时没传 `seed=`，
   模型权重初始化不受 `cfg.seed` 管。证据：训练开始前的 sanity check loss
   就已经不同（5.147 vs 5.001）。
3. **14 个点不显著。** z = 1.44，p = 0.15，95% CI [-5, +33] 跨过 0。

所以它实际上是一次意外的**重跑方差测量**：同架构、同数据、同配方、同预算，
仅随机初始化不同 → 成功率相差 14 个百分点。

这反而给了整个仓库一把尺子：**15 个百分点以内的成功率差距都不能当结论。**
完整检验见 [`04_analysis/significance.py`](04_analysis/significance.py)。
...
而同一协议下发布的 checkpoint 能到 86%——所以这些模型全都欠训练。
六组成功率对比里**只有一组是统计显著的**（官方 checkpoint 对我的，p<0.001），
其余全部跨过 0；经验噪声底约 15 个百分点。
实验二更是直接搞错了——我以为在比架构，其实两者在这个包里是同一个实现。
