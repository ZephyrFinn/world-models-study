...
NOTES = {
 ...
 9: "70秒。这页是主动认错，讲法很关键——不要辩解，要干脆。\n"
    "「我原本想做架构对比。跑出来PLDM预测更差却规划更好，我一度以为抓到了一个漂亮的反例。」\n"
    "【停】「但它不成立。」\n"
    "三条依次说：(1)diff发现两个module.py逐字节相同，从来就不是架构对比；"
    "(2)train.py没给Manager传seed，两次初始权重不同——训练开始前sanity loss就是5.147 vs 5.001；"
    "(3)14个点 z=1.44 p=0.15，CI跨过0。\n"
    "【转折，这才是价值】「所以它实际测的是重跑方差：同架构同数据同配方，只是初始化不同，"
    "成功率就能差14个点。这反而给了我一把尺子——15个点以内的差距都不能当结论，"
    "包括我自己另外两组实验里的差距。」\n"
    "如果被问「那你怎么发现的」：老实说是被问到「官方不是说LeWM比PLDM好吗」之后才去diff的。"
    "这个回答比假装自己主动发现更可信。",
 10: "60秒。全场高潮，语速放慢。\n"
 ...
 15: "50秒。决定最后印象。要讲得干脆，不要显得沮丧。\n"
     "站得住的只有一样：塌缩本身。embedding标准差差270倍、死维度69对0——直接测量，没有噪声解释空间。\n"
     "站不住的：六组成功率对比只有一组显著，噪声底约15个点，连SIGReg那18个点都是p=0.064。\n"
     "实验二更是直接搞错了——以为在比架构，其实是同一个实现。\n"
     "【关键的收尾】「所以我下一步的优先级被这个错误改写了：不是去扫更多超参数，"
     "而是先把评估规模做够——3到5个seed、每次200条以上episode。在那之前，扫什么都分辨不出来。」\n"
     "这句话比任何实验结果都能说明你懂怎么做研究。",
}
...
def build():
    ...
    # 9 实验二
    s = nxt()
    heading(s, "实验二 —— 这一组我做错了")
    tf = tb(s, M, Inches(1.45), CW, Inches(0.6))
    para(tf, "原计划：固定预算只换架构，把 PLDM 塞进同一个训练循环和规划器。"
             "跑出来 PLDM 预测更差却规划更好，看着像个漂亮的反例。", 13, INK2, first=True)
    table(s, [["", "pred_loss", "规划成功率", "死维度"],
              ["LeWM h=3", "0.266", "52%", "0 / 192"],
              ["PLDM", "0.298（更差）", "66%（更好）", "0 / 192"]],
          M, Inches(2.25), Inches(8.4), col_w=[3, 2.4, 2.4, 2], fs=12)
    tf = tb(s, M, Inches(4.0), CW, Inches(2.3))
    para(tf, "但它不成立。事后 diff 发现三件事：", 14, BAD, bold=True, first=True, space_after=6)
    para(tf, "1.  pldm/module.py 和 lewm/module.py 逐字节相同——从来就不是架构对比",
         12.5, INK2, space_after=4)
    para(tf, "2.  train.py 没给 Manager 传 seed，两次初始权重不同"
             "（训练开始前 sanity loss 就是 5.147 vs 5.001）", 12.5, INK2, space_after=4)
    para(tf, "3.  14 个点：z = 1.44，p = 0.15，95% CI [-5, +33] 跨过 0", 12.5, INK2, space_after=10)
    para(tf, "所以它实际是一次意外的重跑方差测量：同架构、同数据、同配方，仅初始化不同，"
             "成功率相差 14 个百分点。这给了整个项目一把尺子——15 个百分点以内的差距都不能当结论。",
         12, DIM)
    ...
    for i, (label, items, acc) in enumerate([
        ("站得住", ["塌缩本身：embedding 标准差 0.0012 对 0.325，差 270 倍；死维度 69 对 0。"
                 "这是直接测量，不是统计推断，没有噪声解释空间。",
                 "官方 checkpoint 的 86%，对比我 900 步训练的所有模型（p<0.001，唯一显著的一组）。"], True),
        ("站不住", ["六组成功率对比里只有一组显著，经验噪声底约 15 个百分点——"
                 "包括 SIGReg 那组的 18 个点（p=0.064）。",
                 "实验二我直接搞错了：以为在比架构，其实两者是同一个实现。",
    ...
    tf = tb(s, M, Inches(4.95), CW, Inches(1.4))
    para(tf, "下一步的优先级被实验二改写了：不是扫更多超参数，而是先把评估规模做够——"
             "3–5 个 seed、每次 200 条以上 episode。在那之前，扫什么都分辨不出来。",
         13, INK2, first=True)
