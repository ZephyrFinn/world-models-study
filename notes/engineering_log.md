...
**把噪声当成了发现，然后发现问题比想的更深。** 实验二原本是要比架构：
把 PLDM 塞进 LeWM 的训练循环，同预算跑一遍。结果 PLDM 预测更差却规划更好，
我当成了"loss 不能给模型排序"的直接反例，写进了 README 和讲稿。

被问到"官方不是宣称 LeWM 比 PLDM 好吗"之后去查，才想明白第一层错误：
**JEPA 这一族方法共享架构，PLDM 和 LeWM 的区别在 loss 上。**
PLDM 用 VCReg×4 + 时序对齐 + 逆动力学（六项），LeWM 用 SIGReg（一项）——
正对上论文摘要那句 "from six to one compared to the only existing end-to-end
alternative"。我只改了模型类的 `_target_`、沿用 LeWM 的 loss，等于把 PLDM 抽空了。
两组训练日志里都有 `sigreg_loss`，就是铁证。

同时发现第二个 bug：`train.py` 调 `spt.Manager` 时没传 `seed=`，权重初始化不受
`cfg.seed` 控制（库自己会警告）。训练开始前的 sanity loss 就已经不同：5.147 对 5.001。

重做时把两个都修了：写 `exp2_train.py` 换真正的变量（防塌缩机制），
并把 seed 传进 Manager。每组 3 个 seed。

**重做的结果比原来的错误更让人不安**：同配置、只换种子，成功率 36% / 60% / 70%，
组内标准差 ±17 个百分点。按这个方差，检出 10 个点的真实差异需要每组约 45 个 seed。
我的实验一和实验三每组只有 1 个——**整个实验设计欠功效约一个数量级**，
而我是做完三组实验之后才知道的。

重做还顺带暴露了第三件事：**`pred_loss` 跨模型不可比。**
PLDM 的预测 loss 稳定低 13-16 倍，但它的 embedding 尺度也小得多；
按尺度平方归一化后两者持平（2.84 对 2.66）。这是塌缩陷阱的弱化版——
不需要真塌缩，只要正则项把表征压紧，MSE 就无偿变好看。
`probe_collapse.py` 现在会输出 `scale_sq` 就是为了随手做这个归一化。

三条教训：想比架构先 `diff` 实现；先搞清楚这一族方法的分类维度是什么；
**评估规模要先于实验设计确定**——花 20 分钟跑 3 个种子量方差，
就能提前知道后面三组实验根本测不出东西。
